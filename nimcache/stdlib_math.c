/* Generated by Nim Compiler v0.10.2 */
/*   (c) 2014 Andreas Rumpf */
/* The generated code is subject to the original license. */
/* Compiled for: Windows, i386, gcc */
/* Command for C compiler:
   F:\Nim\dist\mingw\bin\gcc.exe -c  -w  -IF:\Nim\lib -o f:\mydocument\[programing]\python\pynimcffi\nimcache\stdlib_math.o f:\mydocument\[programing]\python\pynimcffi\nimcache\stdlib_math.c */
#define NIM_INTBITS 32
#include "nimbase.h"

#include <math.h>

#include <stdlib.h>
typedef struct runningstat111844 runningstat111844;
struct  runningstat111844  {
NI N;
NF Sum;
NF Min;
NF Max;
NF Mean;
NF Oldm;
NF Olds;
NF News;
};
N_NIMCALL(NU8, classify_110226)(NF x);
N_NIMCALL(NI, binom_110242)(NI n, NI k);
N_NIMCALL(NI, fac_110281)(NI n);
N_NIMCALL(NIM_BOOL, ispoweroftwo_110318)(NI x);
N_NIMCALL(NI, nextpoweroftwo_110328)(NI x);
N_NIMCALL(NI, countbits32_110828)(NI32 n);
N_NIMCALL(NF, sum_110895)(NF* x_110901, NI x_110901Len0);
N_NIMCALL(NF, mean_110888)(NF* x, NI xLen0);
N_NIMCALL(NF, variance_110976)(NF* x, NI xLen0);
N_NIMCALL(NF, log2_111055)(NF x);
N_NIMCALL(NF, random_111029)(NF max);
N_NIMCALL(void, randomize_111035)(void);
N_NIMCALL(void, randomize_111040)(NI seed);
N_NIMCALL(NF, ntepochTime)(void);
N_NIMCALL(NI, random_111023)(NI max);
N_NIMCALL(NF, mod_111804)(NF x, NF y);
N_NIMCALL(void, push_111861)(runningstat111844* s, NF x);
N_NIMCALL(void, push_111885)(runningstat111844* s, NI x);
N_NIMCALL(NF, variance_111896)(runningstat111844* s);
N_NIMCALL(NF, standarddeviation_111907)(runningstat111844* s);
static N_INLINE(void, nimFrame)(TFrame* s);
N_NOINLINE(void, stackoverflow_18801)(void);
static N_INLINE(void, popFrame)(void);
extern TFrame* frameptr_16242;

N_NIMCALL(NU8, classify_110226)(NF x) {
	NU8 result;
	result = 0;
	{
		if (!(x == 0.0)) goto LA3;
		{
			if (!(((NF)(1.0000000000000000e+000) / (NF)(x)) == INF)) goto LA7;
			result = ((NU8) 2);
			goto BeforeRet;
		}
		goto LA5;
		LA7: ;
		{
			result = ((NU8) 3);
			goto BeforeRet;
		}
		LA5: ;
	}
	LA3: ;
	{
		if (!(((NF)(x) * (NF)(5.0000000000000000e-001)) == x)) goto LA12;
		{
			if (!(0.0 < x)) goto LA16;
			result = ((NU8) 5);
			goto BeforeRet;
		}
		goto LA14;
		LA16: ;
		{
			result = ((NU8) 6);
			goto BeforeRet;
		}
		LA14: ;
	}
	LA12: ;
	{
		if (!!((x == x))) goto LA21;
		result = ((NU8) 4);
		goto BeforeRet;
	}
	LA21: ;
	result = ((NU8) 0);
	goto BeforeRet;
	BeforeRet: ;
	return result;
}

N_NIMCALL(NI, binom_110242)(NI n, NI k) {
	NI result;
	result = 0;
	{
		if (!(k <= 0)) goto LA3;
		result = 1;
		goto BeforeRet;
	}
	LA3: ;
	{
		if (!(n < (NI32)(2 * k))) goto LA7;
		result = binom_110242(n, (NI32)(n - k));
		goto BeforeRet;
	}
	LA7: ;
	result = n;
	{
		NI i_110260;
		NI res_110264;
		i_110260 = 0;
		res_110264 = 2;
		{
			while (1) {
				if (!(res_110264 <= k)) goto LA11;
				i_110260 = res_110264;
				result = (NI32)((NI32)(result * (NI32)((NI32)(n + 1) - i_110260)) / i_110260);
				res_110264 += 1;
			} LA11: ;
		}
	}
	BeforeRet: ;
	return result;
}

N_NIMCALL(NI, fac_110281)(NI n) {
	NI result;
	result = 0;
	result = 1;
	{
		NI i_110297;
		NI res_110301;
		i_110297 = 0;
		res_110301 = 2;
		{
			while (1) {
				if (!(res_110301 <= n)) goto LA3;
				i_110297 = res_110301;
				result = (NI32)(result * i_110297);
				res_110301 += 1;
			} LA3: ;
		}
	}
	return result;
}

N_NIMCALL(NIM_BOOL, ispoweroftwo_110318)(NI x) {
	NIM_BOOL result;
	NIM_BOOL LOC1;
	result = 0;
	LOC1 = 0;
	LOC1 = !((x == 0));
	if (!(LOC1)) goto LA2;
	LOC1 = ((NI)(x & (NI32)(x - 1)) == 0);
	LA2: ;
	result = LOC1;
	goto BeforeRet;
	BeforeRet: ;
	return result;
}

N_NIMCALL(NI, nextpoweroftwo_110328)(NI x) {
	NI result;
	result = 0;
	result = (NI32)(x - 1);
	result = (NI)(result | (NI)((NU32)(result) >> (NU32)(4)));
	result = (NI)(result | (NI)((NU32)(result) >> (NU32)(2)));
	result = (NI)(result | (NI)((NU32)(result) >> (NU32)(1)));
	result += ((NI) ((NI32)(1 + ((NI) ((x <= 0))))));
	return result;
}

N_NIMCALL(NI, countbits32_110828)(NI32 n) {
	NI result;
	NI32 v;
	result = 0;
	v = n;
	v = (NI32)((NU32)(v) - (NU32)((NI32)((NI32)((NU32)(v) >> (NU32)(((NI32) 1))) & ((NI32) 1431655765))));
	v = (NI32)((NU32)((NI32)(v & ((NI32) 858993459))) + (NU32)((NI32)((NI32)((NU32)(v) >> (NU32)(((NI32) 2))) & ((NI32) 858993459))));
	result = ((NI) ((NI32)((NU32)((NI32)((NU32)((NI32)((NI32)((NU32)(v) + (NU32)((NI32)((NU32)(v) >> (NU32)(((NI32) 4))))) & ((NI32) 252645135))) * (NU32)(((NI32) 16843009)))) >> (NU32)(((NI32) 24)))));
	return result;
}

N_NIMCALL(NF, sum_110895)(NF* x_110901, NI x_110901Len0) {
	NF result;
	result = 0;
	{
		NF i_110946;
		NI i_110950;
		i_110946 = 0;
		i_110950 = 0;
		{
			while (1) {
				if (!(i_110950 < x_110901Len0)) goto LA3;
				i_110946 = x_110901[i_110950];
				result = ((NF)(result) + (NF)(i_110946));
				i_110950 += 1;
			} LA3: ;
		}
	}
	return result;
}

N_NIMCALL(NF, mean_110888)(NF* x, NI xLen0) {
	NF result;
	NF LOC1;
	result = 0;
	LOC1 = 0;
	LOC1 = sum_110895(x, xLen0);
	result = ((NF)(LOC1) / (NF)(((double) (xLen0))));
	return result;
}

N_NIMCALL(NF, variance_110976)(NF* x, NI xLen0) {
	NF result;
	NF m;
	result = 0;
	result = 0.0;
	m = mean_110888(x, xLen0);
	{
		NI i_110993;
		NI HEX3Atmp_111003;
		NI res_111006;
		i_110993 = 0;
		HEX3Atmp_111003 = 0;
		HEX3Atmp_111003 = (xLen0-1);
		res_111006 = 0;
		{
			while (1) {
				NF diff;
				if (!(res_111006 <= HEX3Atmp_111003)) goto LA3;
				i_110993 = res_111006;
				diff = ((NF)(x[i_110993]) - (NF)(m));
				result = ((NF)(result) + (NF)(((NF)(diff) * (NF)(diff))));
				res_111006 += 1;
			} LA3: ;
		}
	}
	result = ((NF)(result) / (NF)(((double) (xLen0))));
	return result;
}

N_NIMCALL(NF, log2_111055)(NF x) {
	NF result;
	NF LOC1;
	NF LOC2;
	result = 0;
	LOC1 = 0;
	LOC1 = log(x);
	LOC2 = 0;
	LOC2 = log(2.0000000000000000e+000);
	result = ((NF)(LOC1) / (NF)(LOC2));
	goto BeforeRet;
	BeforeRet: ;
	return result;
}

N_NIMCALL(NF, random_111029)(NF max) {
	NF result;
	int LOC1;
	result = 0;
	LOC1 = 0;
	LOC1 = rand();
	result = ((NF)(((NF)(((NF) (LOC1))) / (NF)(3.2767000000000000e+004))) * (NF)(max));
	return result;
}

N_NIMCALL(void, randomize_111040)(NI seed) {
	srand(((int) (seed)));
}

N_NIMCALL(void, randomize_111035)(void) {
	union { NF source; NI dest; } LOC1;
	LOC1.source = ntepochTime();
	randomize_111040(LOC1.dest);
}

N_NIMCALL(NI, random_111023)(NI max) {
	NI result;
	int LOC1;
	result = 0;
	LOC1 = 0;
	LOC1 = rand();
	result = (NI32)(((NI) (LOC1)) % max);
	return result;
}

N_NIMCALL(NF, mod_111804)(NF x, NF y) {
	NF result;
	result = 0;
	{
		if (!(y == 0.0)) goto LA3;
		result = x;
	}
	goto LA1;
	LA3: ;
	{
		NF LOC6;
		LOC6 = 0;
		LOC6 = floor(((NF)(x) / (NF)(y)));
		result = ((NF)(x) - (NF)(((NF)(y) * (NF)(LOC6))));
	}
	LA1: ;
	return result;
}

N_NIMCALL(void, push_111861)(runningstat111844* s, NF x) {
	(*s).N += 1;
	{
		if (!((*s).N == 1)) goto LA3;
		(*s).Min = x;
		(*s).Max = x;
		(*s).Oldm = x;
		(*s).Mean = x;
		(*s).Olds = 0.0;
	}
	goto LA1;
	LA3: ;
	{
		{
			if (!(x < (*s).Min)) goto LA8;
			(*s).Min = x;
		}
		LA8: ;
		{
			if (!((*s).Max < x)) goto LA12;
			(*s).Max = x;
		}
		LA12: ;
		(*s).Mean = ((NF)((*s).Oldm) + (NF)(((NF)(((NF)(x) - (NF)((*s).Oldm))) / (NF)(((double) ((*s).N))))));
		(*s).News = ((NF)((*s).Olds) + (NF)(((NF)(((NF)(x) - (NF)((*s).Oldm))) * (NF)(((NF)(x) - (NF)((*s).Mean))))));
		(*s).Oldm = (*s).Mean;
		(*s).Olds = (*s).News;
	}
	LA1: ;
	(*s).Sum = ((NF)((*s).Sum) + (NF)(x));
}

N_NIMCALL(void, push_111885)(runningstat111844* s, NI x) {
	push_111861(s, ((double) (x)));
}

N_NIMCALL(NF, variance_111896)(runningstat111844* s) {
	NF result;
	result = 0;
	{
		if (!(1 < (*s).N)) goto LA3;
		result = ((NF)((*s).News) / (NF)(((double) ((NI32)((*s).N - 1)))));
	}
	LA3: ;
	return result;
}

N_NIMCALL(NF, standarddeviation_111907)(runningstat111844* s) {
	NF result;
	NF LOC1;
	result = 0;
	LOC1 = 0;
	LOC1 = variance_111896(s);
	result = sqrt(LOC1);
	return result;
}

static N_INLINE(void, nimFrame)(TFrame* s) {
	NI LOC1;
	LOC1 = 0;
	{
		if (!(frameptr_16242 == NIM_NIL)) goto LA4;
		LOC1 = 0;
	}
	goto LA2;
	LA4: ;
	{
		LOC1 = ((NI) ((NI16)((*frameptr_16242).calldepth + ((NI16) 1))));
	}
	LA2: ;
	(*s).calldepth = ((NI16) (LOC1));
	(*s).prev = frameptr_16242;
	frameptr_16242 = s;
	{
		if (!((*s).calldepth == ((NI16) 2000))) goto LA9;
		stackoverflow_18801();
	}
	LA9: ;
}

static N_INLINE(void, popFrame)(void) {
	frameptr_16242 = (*frameptr_16242).prev;
}
NIM_EXTERNC N_NOINLINE(void, stdlib_mathInit)(void) {
	nimfr("math", "math.nim")
	popFrame();
}

NIM_EXTERNC N_NOINLINE(void, stdlib_mathDatInit)(void) {
}

